...
ACC2020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
Big122020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
Big102020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
SEC2020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
Pac122020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
SoCon2020 <- read.table(pipe("pbpaste"), sep="\t", header = TRUE)
...
#ACC<-
  #ACC %>%
  #full_join(ACC2014, by="Team")
#I didn't inner join ACC 2014 because there were different teams in the conference for this year so inner join wouldn't work...
...
write_csv(ACC, "file=ACC_total.csv")
write_csv(SEC, "file=SEC_total.csv")
write_csv(Big10, "file=Big10_total.csv")
write_csv(Big12, "file=Big12_total.csv")
write_csv(Pac12, "file=Pac12_total.csv")
